...
func testDiscountToManyDiscountAttributes(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	queries.Assign(&b.DiscountID, a.ID)
	queries.Assign(&c.DiscountID, a.ID)
	...
	for _, v := range check {
		if queries.Equal(v.DiscountID, b.DiscountID) {
			bFound = true
		}
		if queries.Equal(v.DiscountID, c.DiscountID) {
			cFound = true
		}
	}
	...
	if err = a.L.LoadDiscountAttributes(ctx, tx, false, (*[]*Discount)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testDiscountToManyAddOpDiscountAttributes(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		...
		if !queries.Equal(a.ID, first.DiscountID) {
			t.Error("foreign key was wrong value", a.ID, first.DiscountID)
		}
		if !queries.Equal(a.ID, second.DiscountID) {
			t.Error("foreign key was wrong value", a.ID, second.DiscountID)
		}

		if first.R.Discount != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}
		if second.R.Discount != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}
...
func testDiscountToManySetOpDiscountAttributes(t *testing.T) {
	var err error
	...
	if !queries.IsValuerNil(b.DiscountID) {
		t.Error("want b's foreign key value to be nil")
	}
	if !queries.IsValuerNil(c.DiscountID) {
		t.Error("want c's foreign key value to be nil")
	}
	if !queries.Equal(a.ID, d.DiscountID) {
		t.Error("foreign key was wrong value", a.ID, d.DiscountID)
	}
	if !queries.Equal(a.ID, e.DiscountID) {
		t.Error("foreign key was wrong value", a.ID, e.DiscountID)
	}
...
func testDiscountToManyRemoveOpDiscountAttributes(t *testing.T) {
	var err error
...
var (
	discountDBTypes = map[string]string{`ID`: `int`, `UserID`: `varchar`}
	_               = bytes.MinRead
)
